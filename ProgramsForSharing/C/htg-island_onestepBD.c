...
void ChooseB(void)
{
  ...
  for(ideme=0; ideme<totpopsize; ideme++) // For each node,
  {
...
void ChooseD(void)
{
  cumD = 1.0; // The out degree is 1 in this model
  ...
  for(ideme=0; ideme<NDEMES; ideme++)
  {
    if(ideme==nodeB){
      ...
      cumsum += ((double) demesize[ideme]) * MIGRATION/(totpopsize - demesize[nodeB]);
    }
    if(randnum < cumsum){
  ...
  randnum = drand48()*demesize[nodeD];
