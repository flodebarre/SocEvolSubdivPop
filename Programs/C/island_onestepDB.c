...
void ChooseD(void)
{
  ...
  {
    cumsum += 1.0; // Update the cumulative sum up to inode
...
void ChooseB(void)
{
  ...
  for(ideme=0; ideme<NDEMES; ideme++) // For each deme
  {
    if(ideme==nodeD){
      ...
      migproba = ((double) MIGRATION /(NDEMES-1) );
    }

    fecundityA[ideme] = migproba * (1.0 - OMEGA * COST + OMEGA * BENEFIT * (pop[ideme] - noselfinteraction)/(DEMESIZE - noselfinteraction)) * pop[ideme]; // For A
