...
muts <- c(0.0000000001, 0.001, 0.01, 0.1)
thed <- 15 # Number of demes in the population 

colmut <- c(colnomut, rev(mygradient4[2:4]))
ltys <- c(2, rep(1, 3))
...
plotlinesQ <- function(LC){
  ...
  xleg <- 0.5
  yleg <- 0.9
  legend(xleg, yleg, col = colmut, legend = round(muts, 5), lty = ltys, lwd = rep(lwdfig[1], 3), bty = "n", cex = 0.9)
