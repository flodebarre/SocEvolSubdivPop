...
## PLOTTING
  ...
  # nA / (nA + nB)
PlotProp <- function(upd, sel, htg, ylim=c(0,1), addAnalysis=FALSE, pdf = TRUE, addTitle = FALSE, plotData = TRUE, muL = mutList, migL = migList, ids = 1, ies = 0, addMu0 = FALSE, theg = 0, sameDE = FALSE, suffix = "", addLegend = FALSE){
  ...
  # suffix suffix for the file name
    ...
    if(pdf){
      ...
      filename <- paste0(prefix, "EX", upd, "_sel", sel, "_htg", htg, ext, DEindic, suffix) # Name of the pdf file

      pdf(paste0(filename, ".pdf"), width = 6., height = 5., compress = FALSE) # Open pdf
    ...
    if(addMu0){
      ...
      curve(tmpP, from=0.001, to=0.9,#par("usr")[2], 
            col=colnomut, add = TRUE, lwd = 2.5, lty = 2)
    }
...
# FIGURE 2
PlotProp("DB", 0.005, 0, addAnalysis = FALSE, ylim = ylm, muL = themutList, migL = themigList, addMu0 = TRUE, plotData = FALSE, suffix = "_justMu0", addLegend = FALSE)

for(i in rev(seq_len(length(mutList)))){
PlotProp("DB", 0.005, 0, addAnalysis = TRUE, ylim = ylm, muL = themutList[i:length(themutList)], migL = themigList, addMu0 = TRUE, plotData = TRUE, suffix = paste0("_", 1+length(mutList)-i), addLegend = TRUE)
}
