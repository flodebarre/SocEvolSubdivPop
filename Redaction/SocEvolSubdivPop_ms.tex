...
To explain how the frequency of altruists can increase with the emigration probability $m$, let us go back to the decomposition of the expected frequency of altruists in the population $\Esp{\overline{X}}$ into different terms (equation~\ref{eq:EXapprox}). For all the life-cycles that we consider, the direct effect of helping others ($\beta_{\direct}$) decreases with emigration $m$, while the direct effect of the cost of helping ($\gamma_{\direct}$) does not change with $m$. If we considered only direct effects, \todo{ici}
, we note that the increase of $\Esp{\overline{X}}$ with $m$ is driven by $\beta_{\indirect}$, that scales the indirect consequences of helping others. 
...
% Allen et al
\Citet{Allen2012} investigated the evolution of altruism on graph-structured populations for any mutation probability, and used a . 
