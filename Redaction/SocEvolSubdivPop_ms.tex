...
Let $Y$ be the type of a randomly chosen individual  in the population, and let $Y'$ be the type of a randomly chosen individual at the next time step. Given a frequency $\nu$ of altruists in the population, we have
\begin{subequations}
\begin{align}
\Esp{Y} &= \nu,\\
\Esp{Y'} &= \nu (1-\mu_{1\to 0}) + (1-\nu) \mu_{0\to 1}.
...
\begin{equation}
\nu = \frac{\mu_{0\to 1}}{\mu_{1\to 0 }+\mu_{0\to 1}}. \label{eq:app:nuformula}
\end{equation}
...
\end{displaymath}
using the formulas \eqref{eq:app:Cov}--(\ref{eq:app:SD2}), and replacing $\nu$ by its value (mutation-drift equilibrium, \eqref{eq:app:nuformula}), we obtain
\begin{equation}
\mathrm{Corr}\left[ Y Y' \right] = 1 - (\mu_{1\to 0} + \mu_{0\to 1}).
\end{equation}
\clearpage
