...
\begin{tikzpicture}
\node[text width=\textwidth, font=\Large, align=center, color=maincol](tit){Fidelity of parent-offspring transmission and the evolution of social
...
\includegraphics[width=0.8\textwidth]{../Pics/logo_cimi.pdf}
\end{center}
\end{frame}
...
\only<5>{
\node[draw, fill=white] at(c1){\includegraphics[width = 0.7\textwidth]{../Pics/Taylor1992.pdf}};
}
\end{tikzpicture}
}
\end{center}
...
\begin{tikzpicture}
\def \wpic {1cm}
\tikzstyle{lc}=[font=\large, minimum height = 1cm, draw=none, text depth = 0.2cm]
\def \dx {0.5cm}
...
\end{tikzpicture}
\end{center}
\footnote{\citeref{Ohtsuki et al. (2006); Taylor, Day \& Wild (2007); Taylor et al. (2010)}}
...
\begin{frame}{Population structures}

Population of fixed size $N$


\def \hh {0.25\textwidth}
\def \hhh {0.45\textwidth}

\uncover<2->{
\begin{columns}[b]
\begin{column}{0.4\textwidth}
\begin{block}{Dispersal graph}
\begin{displaymath}
\mathcal{D}=(d_{ij})_{1\leq i,j\leq N}
\end{displaymath}
{\makebox[\columnwidth]{ \footnotesize $\sum_{i=1}^{N} d_{ij} = \sum_{j=1}^N d_{ji} = 1$.}}
\end{block}
\end{column}
\uncover<4->{
\begin{column}{0.20\textwidth}
\includegraphics[width=\columnwidth]{../Pics/G12_F.pdf}
\end{column}
}
%
\uncover<5->{
\begin{column}{0.20\textwidth}
\includegraphics[width=\columnwidth]{../Pics/GIM_Dgraph.pdf}
\end{column}
}
\end{columns}
...
\begin{displaymath}
\mathcal{E}=(e_{ij})_{1\leq i,j\leq N}
\end{displaymath}
{\makebox[\columnwidth]{\footnotesize (any)}}
\end{block}
}
\end{column}
\uncover<4->{
...
\uncover<5->{
\begin{column}{0.20\textwidth}
\includegraphics[width=\columnwidth]{../Pics/GIM_Egraph.pdf}
\end{column}
}
\end{columns}
...
\footnote{\citeref{Taylor, Day, Wild (2007)}}
}
\end{frame}
...
\node[bigpop]at(7.5,2)(p4){};
\node[bigpop]at(7.5,-2)(p5){};
\node[bigpop]at(10,0)(p6){};
...
\uncover<3->{
\node[textrec, above = 0cm of Ae, font=\normalsize]{Wright-Fisher};

\node[textrec, below = 0cm of Ce, align=center, font=\normalsize]{Moran process};
...
\tikzstyle{lcstep}=[rectangle, rounded corners, text width=3cm, minimum height=1.cm, align=center, fill=bgcol!20, inner sep=0pt, font=\footnotesize]
...
\node[nd, anchor = base]at(\xcst, 0){$1 + \delta $};
...
\node[nd, anchor = base]at(\xcst, \yy){$1 + \delta $};
...
\item Take expectation and let $t\to \infty$; stationary distribution $\xi$
%
\begin{displaymath}
...
\begin{displaymath}
0 = \frac{\delta}{N} \sum_{i=1}^N \left[  \sum_{X\in\Omega}\frac{\partial W_i}{\partial \delta} X_i \xi(\mathbf{X}, 0, \mu) - \sum_{X \in \Omega} \mu B^* X_i \frac{\partial \xi}{\partial \delta} \right] + O(\delta^2),
\end{displaymath}
which we rewrite as
\begin{displaymath}
\delta \mu B^* \frac{\partial \mathbb{E}[\overline{X}]}{\partial \delta} = \frac{\delta}{N} \, \sum_{i=1}^N\mathbb{E}_0\left[ \frac{\partial W_i}{\partial \delta} X_i \right]  + O(\delta^2).
\end{displaymath}
\item Using partial derivatives: phenotypes
\begin{displaymath}
...
\end{displaymath}
\item Then further decompose with partial derivatives:
\begin{displaymath}
\frac{\partial W_i}{\partial \phi_k} = \sum_{\ell = 1}^N \frac{\partial W_i}{\partial f_{\ell}} \frac{\partial f_{\ell}}{\partial \phi_k}
\end{displaymath}
\begin{displaymath}
\frac{\partial f_{\ell}}{\partial \phi_{\ell}} = - c; \quad
\frac{\partial f_{\ell}}{\partial \phi_{\textsf{in}}} = \frac{b}{n-1} ; \quad
...
\subsection{EX}\begin{frame}<1-8>[label = EXframe]{Expected frequency of altruists in the population}
\begin{center}
\begin{tikzpicture}[node distance = 0.2cm and 0.16cm, anchor = base, baseline = 0pt]
% vertical and horizontal
\tikzstyle{math}=[font=\normalsize, inner sep = 0pt, draw = none, minimum height = 1.2cm]
...
\uncover<6->{
\node[above = 0cm of pM, align = center]{Moran Death-Birth \\($1$ death \& $1$ birth)};
}
\uncover<7>{
...
\node[above = 0cm of pBD, align = center]{Moran Birth-Death \\($1$ birth \& $1$ death)};
...
%\uncover<2->{
%\begin{tikzpicture}
%\node[anchor=center]at(0,0)(abs){\includegraphics[width=\textwidth]{../Pics/Kummerli1.pdf}
%};
%\tikzstyle{exc}=[draw, inner sep = 0pt, drop shadow, line width=1.5pt]
%
%\uncover<3->{
%\node[exc]at(-1,0.85){\includegraphics[width=0.8\textwidth]{../Pics/Kummerli2.pdf}};
%
%\node[exc]at(1,-2){\includegraphics[width=0.8\textwidth]{../Pics/Kummerli3.pdf}};
%}
%\end{tikzpicture}
%}
\begin{block}{}
\centering When strategy transmission is imperfect, too small emigration probabilities can prevent the evolution of altruistic behavior
\end{block}
\end{frame}
...
individuals, for any life-cycle, any regular population structure,  any mutation probability. {\footnotesize \textcolor{mgray}{(D., 2017, JTB)}}

\item<+-> $\mathbb{E}[\overline{X}]>\nu \Leftrightarrow \mathcal{B}\, R > \mathcal{C} \Leftrightarrow b \, \kappa > c$.

\item<+-> In subdivided populations, $\mathbb{E}[\overline{X}]$ can increase with the emigration probability $m$ when strategy transmission is imperfect ($\mu > 0$).  {\footnotesize \textcolor{mgray}{(D., \textit{in review.})}}
...
\begin{tikzpicture}
\node[](anr){\includegraphics[height = 1.2cm]{/home/florencedebarre/Dropbox/Presentations/GlobalPics/logo_anr.pdf}};
\node[below = -0.3cm of anr, font = \scriptsize](anrn){ANR-14-ACHN-0003-01};

\node[right = 0cm of anr, align = center, text width=3cm, font=\small](Ch){ R. Aguil\'ee, \\M. Costa,\\ G. Faye, \\S. Gandon, \\S. Mirrahimi,};

\node[right=-0.5cm of Ch, align =center, font=\small](ML){Ch. Mullon, \\S. Lion};

\node[right = 0cm of ML, color = maincol, font = \normalsize, align = center]{and thank you\\ for your attention!};
